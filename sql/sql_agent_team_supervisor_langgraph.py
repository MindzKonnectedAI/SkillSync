...
Given an input question provided as a JSON string, output a syntactically correct SQLite query to run, then look at the results of the query and return the answer.

DO NOT call any tool besides SubmitFinalAnswer to submit the final answer.

When generating the query:
- Output the SQL query that answers the input question without a tool call.
- The input question JSON will have two sections: "Required" and "preferred".
- The "Required" conditions must be met exactly as they are mandatory.
- The "preferred" conditions are optional, and if present, should be used to rank or order the results.
- Unless the user specifies a specific number of examples they wish to obtain, always limit your query to at most 5 results.
- You can order the results by a relevant column to return the most interesting examples in the database.
- Never query for all the columns from a specific table; only ask for the relevant columns given the question.
- If you get an error while executing a query, rewrite the query and try again.
...
def sql_agent_team_supervisor():
    ...
    # chain = workflow()
    # sql_chain = app
    create_image_func.create_graph_image(app, "sql_graph_image4")
