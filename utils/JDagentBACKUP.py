...
def find_required_point(state):
    ...
    template="""
        ...
        Explanation of Output:
            ###Required:
            Location (required): Extracted the location "New York, NY" (required).
            Experience (required): Extracted "5+ years" (required).
            Graduation (required): Extracted "Bachelor's degree in Computer Science" (required).
            Post Graduation (required): Left as "N/A" since no required postgraduate degree was mentioned.(required)
            PhD (required): Left as "N/A" since no required PhD was mentioned.(required)
            Frontend (required): Extracted "React" (required).
            Backend (required): Extracted "Node.js" (required).
            DB (required): Left as "N/A" since MongoDB was mentioned as a "plus" (required).
            Tools (required): Left as "N/A" since AWS was mentioned as a "plus" (required).
            Miscellaneous (required): Extracted "Knowledge of Agile methodologies, excellent communication skills" (required).
...
def check_required_point(state):
    ...
    template = """
        Objective: Verify that the extracted required points from extracted_required_points align with the job description.
        Identify any missing required points and add them.
        Ensure no preferred, optional, or nice-to-have details are included.
        ...
        Instructions:
        Compare extracted_required_points with job_description to check for accuracy.
        Identify any missing required points and add them to the output.
        Remove incorrect or optional details (e.g., “preferred,” “nice-to-have”).
        Ensure clarity and accuracy by strictly following the job description's language.
        Use "N/A" for fields with no required information.

        Don't add explantion in output only return final results.

        Output Format:
        ###Required:
        Location (required): [Final Verified Required Location]
        Experience (required): [Final Verified Required Experience]
        Graduation (required): [Final Verified Required Graduation]
        Post Graduation (required): [Final Verified Required Post Graduation]
        PhD (required): [Final Verified Required PhD]
        Frontend (required): [Final Verified Required Frontend Skills]
        Backend (required): [Final Verified Required Backend Skills]
        DB (required): [Final Verified Required Database Skills]
        Tools (required): [Final Verified Required Tools]
        Miscellaneous (required): [Final Verified Required Miscellaneous Information]
...
def find_preferred_point(state):
    ...
    template="""  
        ...
        **Explanation of Output:**  
            ###Preferred:  
            Location (preferred): Extracted the preferred location "San Francisco, CA" (preferred).  
            Experience (preferred): Extracted "3+ years of experience in a related field" (preferred).  
            Graduation (preferred): Left as "N/A" since no preferred undergraduate degree was mentioned (preferred).  
            Post Graduation (preferred): Extracted "Master's degree in Computer Science" (preferred).  
            PhD (preferred): Left as "N/A" since no preferred PhD was mentioned (preferred).  
            Frontend (preferred): Extracted "Vue.js, Angular" (preferred).  
            Backend (preferred): Extracted "Django, Ruby on Rails" (preferred).  
            DB (preferred): Extracted "MongoDB, PostgreSQL" (preferred).  
            Tools (preferred): Extracted "AWS, Docker, Kubernetes" (preferred).  
            Miscellaneous (preferred): Extracted "Familiarity with DevOps practices, knowledge of GraphQL" (preferred).  
...
def check_preferred_point(state):
    ...
    #     """
    template = """  
        Objective: Verify that the extracted preferred points from extracted_preferred_points align with the job description.  
        Identify any missing preferred points and add them.  
        Ensure no required or mandatory details are included.  
        ...
        **Instructions:**  
        - Compare extracted_preferred_points with job_description to check for accuracy.  
        - Identify any missing preferred points and add them to the output.  
        - Remove any **required or mandatory** details (e.g., must-have, compulsory).  
        - Ensure clarity and accuracy by strictly following the job description's language.  
        - Use "N/A" for fields with no preferred information.  
        - If the location is not marked as preferred, then it should not be considered as preferred.

        **Don't add explanations in the output—only return the final results.**  

        **Output Format:**  
        ###Preferred:  
        Location (preferred): [Final Verified Preferred Location]  
        Experience (preferred): [Final Verified Preferred Experience]  
        Graduation (preferred): [Final Verified Preferred Graduation]  
        Post Graduation (preferred): [Final Verified Preferred Post Graduation]  
        PhD (preferred): [Final Verified Preferred PhD]  
        Frontend (preferred): [Final Verified Preferred Frontend Skills]  
        Backend (preferred): [Final Verified Preferred Backend Skills]  
        DB (preferred): [Final Verified Preferred Database Skills]  
        Tools (preferred): [Final Verified Preferred Tools]  
        Miscellaneous (preferred): [Final Verified Preferred Miscellaneous Information]  
    """  
