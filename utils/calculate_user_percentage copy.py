...
def calculate_user_points(table, dict_obj):
    ...
    # Helper function to assign 1 point if a match is found
    def assign_point(name, point_type):
        if name not in points[point_type]:
            points[point_type][name] = 0
        points[point_type][name] += 1
    ...
    def calculate_points(name, row, criteria, point_type):
        for header, values in criteria.items():
            if header in headers:
                ...
                cell_value = row[index].strip() if isinstance(row[index], str) else row[index]
                
                # Experience field is a special case
                if header == 'Experience':
                    if cell_value and 2 <= float(cell_value) <= 4:
                        assign_point(name, point_type)
                else:
                    # For other fields, check if there's a match regardless of value type
                    if isinstance(values, list):
                        for value in values:
                            if value.strip().lower() in str(cell_value).strip().lower():
                                assign_point(name, point_type)
                    else:
                        if str(values).strip().lower() == str(cell_value).strip().lower():
                            assign_point(name, point_type)

    # Calculate points for each row (each candidate)
    for row in rows:
        name = row[0]
        # Calculate required points
        calculate_points(name, row, dict_obj.get('Required', {}), 'Required')
...
def calculate_points(dict_obj):
    ...
    # Assign 1 point per field regardless of type
    def assign_one_point(_):
        return 1
    ...
    if 'Preferred' in dict_obj:
        for key, value in dict_obj['Preferred'].items():
            print("preferred key:", key, "preferred value:", value)
            total_preferred_points += assign_one_point(value)

    # Calculate points for the required section
    if 'Required' in dict_obj:
        for key, value in dict_obj['Required'].items():
            print("required key:", key, "required value:", value)
            total_required_points += assign_one_point(value)
